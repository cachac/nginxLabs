...
```
## 4.3. Optional. Set Headers Cache-Control (Client side cache)
...
```
Set to static files location
```
location ~* \.(.css|js|jpg|jpeg|png) {
	add-header Cache-Control: Expires 600
	...
```
...
## 8.1. add new cache path to nginx.conf
```sh
proxy_cache_path /var/cache/micro-nginx levels=1:2 keys_zone=micro-cache:10m max_size=1g inactive=5m use_temp_path=off;
```
- name= micro-cache
- inactive=5m
## 8.2. add new location /micro
```sh
server {
	...
  location /micro {
